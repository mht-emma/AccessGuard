...
async function login(req, res, next) {
  ...
  try {
    ...
    const cypher = `
      ...
      OPTIONAL MATCH (p)-[:ACCESS_TO]->(res:Resource)
      RETURN 
        ID(u) AS userId,
        u.id AS userUuid,
    ...
    const user = {
      ...
      createdAt: record.get('createdAt') ? (record.get('createdAt').toString ? record.get('createdAt').toString() : record.get('createdAt')) : null,
...
async function me(req, res) {
  ...
  const user = req.session.user || {};
  const userId = user.userId || user.id;
  const { username, role, email, createdAt } = user;
